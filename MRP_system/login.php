<?php
    ...
    if(isset($_POST['btn_login'])){
        ...
        $sql_cmd= "SELECT * FROM tbl_employee WHERE username = '".$user_login."' AND password = '".$pass_login."'";
        $query = mysqli_query($mysqli, $sql_cmd);
        if(mysqli_num_rows($query)> 0){
            $_SESSION['dangnhap'] = $user_login;
            $user = $_SESSION['dangnhap'];
            $temp_us = new employee();
            $_SESSION['user'] = $temp_us->getAllWhere('`tbl_employee`', " * ", " `username` =  '$user' "   );
            header('Location:mrp_index.php?action=quanlyphongban&query=lietke');
        } else{
...
<body>
    <div class="wraper">
        ...
        <form action="" method="post" autocomplete="none">
            <div class="box_login">
                    <p class="box_name">LEDVN MRP LOGIN</p>
